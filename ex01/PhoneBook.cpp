...
bool PhoneBook::addContact() {
  ...
  this->contacts[target].fillContact(firstName, lastName, nickname,
...
static int getUserIndex() {
  ...
  std::cout << MAGENTA << "    ==== Please select an index to search for: "
            << RESET;
  ...
  if (userIndex.length() != 1
    || !std::isdigit(static_cast<unsigned char>(userIndex[0]))) {
    std::cout << BOLD << RED
...
void PhoneBook::searchContact() {
  ...
  if ((std::size_t)idx > index - 1) {
    std::cout << BOLD << RED << "    ==== No Contact found in this index ):"
              << RESET << std::endl;
    return;
  }
